...
class Main {
    ...
    public static void bfs() {
        // BFS에서는 노드 번호만 관리하면 되므로 int형 Queue 사용
        ...
        while (!queue.isEmpty()) {
            int cur = queue.poll();
            // A에 도달한 경우에도 역추적을 위해 탐색을 계속할 수 있음
            // if (cur == A) continue; // 문제에 따라 처리

            // cur 노드와 연결된 모든 인접 노드 탐색
            for (Node next : graph.get(cur)) {
                if (!visited[next.num]) {
                    // 조건: next 노드를 거쳐 cur로 도달하는 것이 최단경로라면
